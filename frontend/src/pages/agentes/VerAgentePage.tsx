...
import { ArrowLeft, User, MapPin, DollarSign, Phone, Mail, Calendar } from 'lucide-react'
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-500">
              ...
              {agente.fecha_ingreso && (
                <span className="flex items-center gap-1"><Calendar size={13} /> Ingreso: {new Date(agente.fecha_ingreso + 'T00:00:00').toLocaleDateString('es-PE')}</span>
              )}
            </div>
          </div>
                ...
                return (
                  <tr key={r.id} className={`border-b ${dif < 0 ? 'border-red-100 bg-red-50/30' : 'border-gray-100 hover:bg-gray-50/60'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`inline-block px-2 py-0.5 rounded-md text-xs font-bold ${dif === 0 ? 'bg-gray-100 text-gray-600' : dif < 0 ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {dif > 0 ? '+' : ''}{fmt(dif)}
                      </span>
                    </td>
