...
| T3.2 | **Ajuste maestro de inventario** | `admin_ajuste_inventario.php` (reset a 0 / carga física real + log `AJUSTE`); sin equivalente. |
...
| T3.5 | **Consulta de token activo** | `verificar_token_activo.php` (GET token vigente + tipo diario/permanente); no hay GET, solo generar/revocar. |
| T3.6 | **Recálculo masivo comisiones operativas** | `recalcular_comisiones_masivo.php` recalcula planes Y operativos (recargas/bipay/krece/payjoy); `recalcularMasivo` solo POSTPAGO/PREPAGO. |
| T3.7 | **Stock multi-IMEI + series_info chips** | `guardar_stock.php` soporta `imei_serial[]` (N equipos) y rangos `serie_inicio[]/serie_fin[]` con `series_info` JSON; el nuevo registra 1 ítem y no guarda series_info. |
...
| ID | Gap |
|----|-----|
| T4.1 | **Exports Excel**: estadísticas corporativo (ranking tiendas) solo JSON; asistencias en CSV sin columnas legacy (minutos_deuda, omitio_refrigerio); kardex en CSV vs Excel. |
